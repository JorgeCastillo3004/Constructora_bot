...
def create_vcf_file(csv_file, file_out = 'contacts_file'):
    out_file = file_out +'.vcf'
    res = vcard_generator(csv_file)
...
def create_csv_contacts(file_input, file_out):
    if '.csv' in file_input:
        df = pd.read_csv(file_input)
    else:
        df = pd.read_excel(file_input)
    df_contacts = df[['Asesor','Teléfono','Email']]
    df_contacts = df_contacts.rename(columns={"Asesor": "name", "Teléfono": "phone","Email":'email'})
    df_contacts.to_csv(file_out)